...
export default function EducationSection() {
    return (
        <section id="education" aria-label="Educational background" className="py-20 bg-gradient-to-br from-white via-slate-50 to-slate-200 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-500 rounded-2xl">
                ...
                {education.map((edu, index) => (
                    <div key={index} className="education-item flex-1 p-8 bg-white/90 dark:bg-slate-800/90 border border-primary/10 dark:border-slate-700 rounded-2xl shadow-xl flex flex-col justify-between transition-colors">
                        <div className="flex items-center mb-4">
                            <img src={edu.image} alt={edu.institution} width="128" height="64" loading="lazy" className="w-32 h-16 rounded-xl mr-4 object-cover shadow-md border border-primary/20 bg-white dark:bg-slate-900" />
                            <div>
                                <h2 className='text-xl md:text-2xl font-bold mb-1 text-primary drop-shadow-sm'>{edu.institution}</h2>
                                <p className='text-base md:text-lg mb-1 text-slate-600 dark:text-slate-300 font-medium'>{edu.degree}</p>

...
export default function CertificationsSection() {
    return (
        <section id="certifications" aria-label="Professional certifications, diplomas, and courses" className="py-20 bg-gradient-to-br from-slate-50 via-white to-slate-200 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-500 rounded-2xl">
            ...
            <div className="certification-types grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 max-w-7xl mx-auto px-2 md:px-0">
                        ...
                        <ul className="certification-list flex flex-col gap-6 overflow-y-auto pr-2" style={{ maxHeight: '400px' }}>
                            {items.map((item, index) => (
                                <li key={index} className="mb-0">
                                    ...
                                    <p className='text-sm text-slate-600 dark:text-slate-400 mt-1'>{item.issuer} <span className="mx-1">|</span> {new Date(item.date).toLocaleDateString(undefined, { year: 'numeric', month: 'short' })}</p>
